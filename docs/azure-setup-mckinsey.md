...
This guide is for **Copilot Studio Direct Line + Azure App Registration** on the mckinsey.com tenant. It walks you through creating an Azure/Entra app registration via Platform McKinsey, then configuring API permissions and redirect URLs so you can use the app with Direct Line and this CLI. Content is sourced from McKinsey’s official internal guides (linked at the top); those guides are the source of truth.
...
3. Fill out the form. For **Copilot Studio Direct Line**, use:
   ...
   - **Configure App Registration:**  
     - **What type of App Registration do you need?** **I need a new App Registration** (or update existing if you already have one).  
     - **App Name** (max 50 characters): e.g. `CopilotStudioDirectLine` or a name that identifies this app as used for Direct Line + Copilot Studio.  
     - **Environments:** **Production**.  
     - **Azure Tenant:** **McKinsey.com**.  
     - **Region:** Select the appropriate region for your agent.
   - **Details:**  
     - **Your Project / Product Workspace:** Select the workspace you created or use.  
     - **Contact email address:** Your email—you will receive the **App ID** and **Tenant ID** here when the app is provisioned.  
     - **Description of the Application:** e.g. “Entra ID app for Copilot Studio Direct Line authentication (Direct Line API + CLI).”  
     - **Justification for the request**, **Expected Go-Live date**, and **Notes** as required by the form.
4. Click **Save** to submit the request.

**Note:** A new App Registration can take **several hours** to provision. You will receive an email once it is ready.
...
6. **Grant admin consent for mckinsey.com:** After adding permissions, you **must** click **Grant admin consent for mckinsey.com**. This grants consent for the mckinsey.com tenant so users don’t see consent prompts.

For other MSGraph **Application permissions** (e.g. User.Read.All, Group.Read.All), see the official guides linked at the top.
...
- **Redirect or permission errors:** Confirm redirect URIs, public client flows, Delegated permissions (openid, profile), and **Grant admin consent for mckinsey.com** were applied. See the [official guide for MSGraph permissions](https://platform.mckinsey.com/knowledge-base/service-guide/1560346639/how-to-request-msgraph-permissions-for-app-registration) if needed.
